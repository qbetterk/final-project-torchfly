...
class MasterBar():
    def __init__(self, gen, cls, total=None): self.first_bar = cls(
...
def text2html_table(lines):
    ...
    for line in lines:
        html_code += "  <tr>\n"
        html_code += "\n".join(
            [f"    <th>{o}</th>" for o in line if len(o) >= 1])
        html_code += "\n  </tr>\n"
...
class NBProgressBar(ProgressBar):
    def __init__(self, gen, total=None, display=True, leave=True, parent=None, auto_update=True):
        self.progress = html_progress_bar(
    ...
    def write(self, line, table=False):
        if table:
            ...
            if not hasattr(self, 'names'):
                self.names = [
                    name + ' ' * (8-len(name)) if len(name) < 8 else name for name in line]
